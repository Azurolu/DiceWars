...
//Mon bebe, imbattable
public class BigBrain extends Brain {
	
	 public BigBrain(Joueur AI) {
		 super(AI);
		 this.player = AI;
	 }
	 ...
	 public boolean action(Carte map, Jeu j, int x) {
		 ...
		 for (Territoire[] t0 : territories2) {
			 for (Territoire t1 : t0) {
				 if (t1.getProprio().equals(this.player) && t1.getNbDes()>1) {
					 ...
					 //Les id de territoires et de voisins sont stockees dans une liste
					 for (Territoire n : neighbors2) {
						 if (!(n.getProprio().equals(this.player))) {
							 int[] tmp0 = {t1.getId(), n.getId()};
							 tmp.add(tmp0);
						 }
					 }
				 }
		 ...
		 int[] bestmove = null;
		 for(int[] tmp0 : tmp) {
			 if(formula(map.getTerritoire(tmp0[0]), map.getTerritoire(tmp0[1])) >= bestproba) {
				bestproba = formula(map.getTerritoire(tmp0[0]), map.getTerritoire(tmp0[1]));
				bestmove = tmp0;
			 }
		 }
		 if(bestproba>=0.5) {
			 j.attaquer(bestmove[0], bestmove[1]);
			 return true;
		 }
		 return false;
	 }
